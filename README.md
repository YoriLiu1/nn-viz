# 带 AI 扩展指令的 RISC-V 处理器 —— 实现单层全连接网络（2 输入 → 2 输出）推理并且将其可视化
...
本项目包含可视化页面和配套的汇编测试程序。后续可能开源更多自己的改动，包括基于开源cpu，对其bug以及涉及到的总线进行重构，采用AHB-lite总线以及APB总线，以及扩展的4条指令的实现相关涉及代码。
